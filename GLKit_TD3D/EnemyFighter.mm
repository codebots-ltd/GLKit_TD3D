...
- (id)initWithScene:(SSGameSceneController *)aGameScene {
    ...
    if (self) {
        ...
        // it
        model = [assetManager getModelWithName:@"EnemyFighter"];
        if (!model) {
            [assetManager loadTexturedMeshWithData:EnemyFighterVertexData vertexCount:kEnemyFighterNumberOfVertices textureFileName:@"EnemyFighterTexture.png" scale:1.0f modelName:@"EnemyFighter"];
            model = [assetManager getModelWithName:@"EnemyFighter"];
        }
